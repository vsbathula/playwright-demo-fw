...
export class TestDataManager {
  ...
  private loadTestData(): void {
    if (!fs.existsSync(this.testDataDir)) {
      this.logger.error(
        `Test data directory does not exist: ${this.testDataDir}`,
      );
      throw new Error(
        `Test data directory does not exist: ${this.testDataDir}`,
      );
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          this.logger.error(`Failed to load test data from ${file}: ${error}`);
        }
      }
    } catch (error) {
  ...
  /**
   * Get test data by key for a specific environment
   * @param environment - Environment name (e.g., 'qa', 'int'*)
  ...
  private getNestedValue(obj: any, path: string): any {
    ...
    for (const key of keys) {
      if (current && typeof current === "object" && key in current) {
        ...
        this.logger.error(`Key not found in test data: ${key}`);
        return undefined;
      }
    }
    return current;
